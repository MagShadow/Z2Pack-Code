...
from _utils import nt, convertSpin, settings, settings_strict, CONST_HJZ, CONST_HZL, CONST_HZL_M
...
def Run_HZL(_N, _J, _i, _j, Phase):
    ...
    '''
    print("Start Calc: N=%d, J=%.3f" % (int(_N), _J))
    h = Hamiltonian(N=int(_N), J=_J, Delta=3, **CONST_HZL_M)
    res = Calc(h, CalcZ2=True, LogOut=False, settings=settings)
    Phase[_i][_j] = TopoOrder(res)
...
if __name__ == '__main__':
    ...
    PD_2 = PhaseDiag().run(Run_HZL, 6, 20, 5, 0, 0.1, 6, "N", "J")
