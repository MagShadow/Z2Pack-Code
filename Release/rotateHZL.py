...
if __name__ == '__main__':
    ...
    func1 = partial(Run_Mirror, N=15, CONST=CONST_HZL)
    func2 = partial(Run_Inv, N=15, CONST=CONST_HZL)
    func3 = partial(Run_M_I, N=15, CONST=CONST_HZL)

    PD_1 = PhaseDiag().run(func1, 0, np.pi, 13, 0, 0.03, 15, "Theta", "J")
    PD_1.write(filename=File_1)
    # PD_2 = PhaseDiag().run(func2, 0, np.pi, 13, 0, 0.03, 15, "Theta", "J")
    # PD_2.write(filename=File_2)
    # PD_3 = PhaseDiag().run(func3, 0, np.pi, 13, 0, 0.03, 15, "Theta", "J")
    # PD_3.write(filename=File_3)

    PD_1.draw(title="PhaseDiag of Theta & J, N=15, Delta=$3\AA$, Mirror\nParameters from Hai-Zhou Lu's paper.",
              xlabel="# of Layers", ylabel="$J(\\rm eV)$", filename=File_1)
    # PD_2.draw(title="PhaseDiag of Theta & J, N=15, Delta=$3\AA$, Inversion\nParameters from Hai-Zhou Lu's paper.",
    #           xlabel="# of Layers", ylabel="$J(\\rm eV)$", filename=File_2)
    # PD_3.draw(title="PhaseDiag of Theta & J, N=15, Delta=$3\AA$, Mirror+Inversion\nParameters from Hai-Zhou Lu's paper.",
    #           xlabel="# of Layers", ylabel="$J(\\rm eV)$", filename=File_3)
